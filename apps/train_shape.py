...
def train(opt):
    ...
    optimizerG = torch.optim.RMSprop(
        netG.parameters(), lr=opt.learning_rate, momentum=0, weight_decay=0
    )
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            eta = ((iter_net_time - epoch_start_time) / (train_idx + 1)) * len(
                train_data_loader
            ...
            if train_idx % opt.freq_save == 0 and train_idx != 0:
                ...
                torch.save(
                    netG.state_dict(),
                    "%s/%s/netG_epoch_%d" % (opt.checkpoints_path, opt.name, epoch),
                )
            ...
            if train_idx % opt.freq_save_ply == 0:
                ...
                save_samples_truncted_prob(
                    save_path, points.detach().numpy(), r.detach().numpy()
                )
        ...
        with torch.no_grad():
            ...
            if not opt.no_num_eval:
                ...
                print(
                    "eval test MSE: {0:06f} IOU: {1:06f} prec: {2:06f} recall: {3:06f}".format(
                        *test_errors
                    )
                )
                ...
                print(
                    "eval train MSE: {0:06f} IOU: {1:06f} prec: {2:06f} recall: {3:06f}".format(
                        *train_errors
                    )
                )
