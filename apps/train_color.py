...
def train_color(opt):
    ...
    if opt.continue_train:
        if opt.resume_epoch < 0:
            ...
            model_path_C = "%s/%s/netC_epoch_%d" % (
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if train_idx % opt.freq_save == 0 and train_idx != 0:
                ...
                torch.save(
                    netC.state_dict(),
                    "%s/%s/netC_epoch_%d" % (opt.checkpoints_path, opt.name, epoch),
                )
            ...
            if train_idx % opt.freq_save_ply == 0:
                ...
                save_samples_rgb(
                    save_path, points.detach().numpy(), rgb.detach().numpy()
                )
        ...
        with torch.no_grad():
            ...
            if not opt.no_num_eval:
                ...
                test_color_error = calc_error_color(
                    opt, netG, netC, cuda, test_dataset, 100
                )
