...
class HourGlass(nn.Module):
    ...
    def _generate_network(self, level):
        self.add_module(
            "b1_" + str(level), ConvBlock(self.features, self.features, norm=self.norm)
        )

        self.add_module(
            "b2_" + str(level), ConvBlock(self.features, self.features, norm=self.norm)
        )
        ...
        self.add_module(
            "b3_" + str(level), ConvBlock(self.features, self.features, norm=self.norm)
        )
...
class HGFilter(nn.Module):
    def __init__(self, opt):
        ...
        for hg_module in range(self.num_modules):
            ...
            self.add_module(
                "top_m_" + str(hg_module), ConvBlock(256, 256, self.opt.norm)
            )
            self.add_module(
